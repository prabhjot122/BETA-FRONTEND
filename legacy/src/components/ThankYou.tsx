...
export default function ThankYou() {
  ...
  const state = undefined as unknown as ThankYouState;
  ...
  const isDemoRoute = pathname === '/demo-thank-you';

  // Resolve user name from query, storage, or stored user data
  const resolvedUserName = state?.userName
    || userFromQuery
    || (typeof window !== 'undefined' ? localStorage.getItem('thankYouName') || undefined : undefined)
    || (typeof window !== 'undefined' ? authService.getStoredUser()?.name : undefined)
    || (isDemoRoute ? 'Demo User' : null);

  // If still no user name and not demo route, redirect to home
  useEffect(() => {
    if (!resolvedUserName && !isDemoRoute) {
      router.push('/');
      return;
    }
  }, [resolvedUserName, isDemoRoute, router]);
  ...
  const handleShare = async (platform: 'facebook' | 'twitter' | 'linkedin' | 'instagram') => {
    ...
    try {
      ...
      const shareText = ` Congratulations to ${resolvedUserName} for becoming a beta testing founding member at LawVriksh!
                ...
                <div className="around-me__stats">
                  <div className="around-me__user-card">
                    <div className="around-me__user-info">
                      <h3 className="around-me__user-name">{resolvedUserName}</h3>
